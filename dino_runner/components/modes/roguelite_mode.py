...
class DamageNumber(pygame.sprite.Sprite):
    def __init__(self, x, y, damage, font):
        super().__init__()
        self.image = font.render(str(int(damage)), True, (255, 255, 0))
...
class RogueliteMode:
    ...
    def reset(self):
        ...
        # Atributo para gerir a pausa do cooldown
        self.pause_start_time = 0

        self.define_powerups()
        self.define_enemy_pools()
        self.class_options = [
            {'name': 'Mage', 'desc': 'Dispara bolas de fogo.', 'id': 'pistol'},
            {'name': 'Warrior', 'desc': 'Mais rápido e resistente.', 'id': 'sword'}
        ]
    ...
    def define_powerups(self):
        """Define todos os power-ups com valores rebalanceados."""
        self.powerup_pool = [
            # BALANCEAMENTO: Efeitos de status reduzidos
            {'name': 'Dano Aumentado', 'desc': '+5 de dano', 'effect': self.increase_damage},
            {'name': 'Cadencia de Tiro', 'desc': 'Atire 10% mais rapido', 'effect': self.increase_fire_rate},
            ...
            {'name': 'Quantidade de Ataques', 'desc': '+1 ataque por clique', 'effect': self.increase_quantity},
        ]
        self.mage_powerups = [
            {'name': 'Bala Perfurante', 'desc': '+1 perfuracao', 'effect': self.increase_pierce},
            {'name': 'Velocidade de Projetil', 'desc': '+15% de velocidade', 'effect': self.increase_bullet_speed},
            {'name': 'Ricochete', 'desc': 'Projeteis ricocheteiam +1 vez', 'effect': self.increase_bounces},
        ]
        self.warrior_powerups = [
            {'name': 'Alcance Aumentado', 'desc': '+20% de area de ataque', 'effect': self.increase_sword_size},
        ]
    ...
    def increase_sword_size(self):
        """Power-up que só funciona para a espada."""
        if isinstance(self.player.weapon, Sword):
            self.player.weapon.size += 0.2

    def increase_damage(self):
        # BALANCEAMENTO: Aumento de dano reduzido
    ...
    def increase_fire_rate(self):
        # BALANCEAMENTO: Aumento de velocidade de ataque reduzido
    ...
    def trigger_level_up(self):
        ...
        # Junta os power-ups comuns com os da classe atual
        ...
        if num_samples > 0:
            self.powerup_options = random.sample(current_pool, num_samples)
        else:
    ...
    def select_class(self, class_id):
        if class_id == 'pistol': # Classe Mage
            ...
            # BALANCEAMENTO: Aumentado o cooldown inicial (ataque mais lento)
            self.player.weapon.attack_cooldown = 650 # Aprox. 1.5 ataques/s
            ...
            self.player.speed = 7
            # BALANCEAMENTO: Vida inicial reduzida
    ...
    def apply_powerup(self, index):
        """Aplica o efeito do power-up e ajusta o cooldown."""
        # Calcula a duração da pausa
        paused_duration = pygame.time.get_ticks() - self.pause_start_time
        # Adiciona a duração da pausa ao temporizador da habilidade
        self.player.last_special_ability_time += paused_duration
        
        chosen_powerup = self.powerup_options[index]
        chosen_powerup['effect']()
        self.game_state = "RUNNING"
        self.selected_option_index = None
    ...
    def draw_card_screen(self, title, options):
        ...
        for i, option in enumerate(options):
            ...
            # CORREÇÃO: Borda dourada removida. Todas as cartas têm a mesma borda azul.
    ...
    def update(self):
        ...
        # Atualiza inimigos e verifica colisão física
        for enemy in self.enemies:
        ...
        for proj in self.projectiles[:]:
            ...
            for enemy in self.enemies[:]:
                if enemy.hitbox.colliderect(proj.hitbox):
                    if proj.is_frozen:
                        enemy.apply_slow(0.75, 2000) # Reduz a velocidade para 75% por 2s
                    ...
                    if proj.pierce <= 0:
                        ...
                        if proj.bounces_left > 0:
                            proj.bounces_left -= 1
                            proj.pierce = 1 # Renova a perfuração para o próximo alvo
                            # Encontra o inimigo mais próximo que não seja o atual
                            ...
                            else: # Sem mais alvos, remove o projétil
                                if proj in self.projectiles: self.projectiles.remove(proj)
                        else: # Sem mais ricochetes, remove
                            if proj in self.projectiles: self.projectiles.remove(proj)
                        break
    ...
    def find_next_bounce_target(self, current_pos, last_hit_enemy):
        """Encontra o inimigo mais próximo para o ricochete."""
        closest_enemy = None
        min_dist = float('inf')
        for enemy in self.enemies:
    ...
    def player_attack_logic(self):
        """A lógica de ataque agora apenas lida com o resultado do ataque."""
        attack_result = self.player.attack()
        if not attack_result: return
        
        # A arma agora lida com o seu próprio som.
        # Esta lógica apenas cria o projétil se ele for retornado.
        if isinstance(attack_result, tuple) and attack_result[0] == "BULLET":
            _, x, y, direction, is_frozen = attack_result
            self.projectiles.append(Bullet(x, y, direction, self.assets, is_frozen))
    ...
    def draw(self):
        ...
        if self.game_state != "CHOOSE_WEAPON":
            ...
            #pygame.draw.rect(self.screen, (0, 255, 0), self.player.hitbox.move(render_offset), 2)
            
            for enemy in self.enemies: 
                enemy.draw(self.screen, render_offset)
                #pygame.draw.rect(self.screen, (255, 0, 0), enemy.hitbox.move(render_offset), 2)

            for proj in self.projectiles: 
                proj.draw(self.screen, render_offset)
                #pygame.draw.rect(self.screen, (255, 255, 0), proj.hitbox.move(render_offset), 2)
            
            for proj in self.enemy_projectiles:
                proj.draw(self.screen, render_offset)
                #pygame.draw.rect(self.screen, (255, 0, 255), proj.hitbox.move(render_offset), 2)
    ...
    def apply_powerup(self, index):
        """Aplica o efeito do power-up e o regista se for único."""
        chosen_powerup = self.powerup_options[index]
        chosen_powerup['effect']()
        
        # Se for um power-up do tipo 'skill', adiciona à lista de adquiridos
        if chosen_powerup.get('type') == 'skill':
            self.acquired_skills.add(chosen_powerup['name'])
    ...
    def draw_ui(self):
        ...
        hp_text = self.ui_font.render(f"HP: {int(self.player.health)}/{self.player.max_health}", True, (255, 255, 255))
        self.screen.blit(hp_text, hp_text.get_rect(center=hp_bar_rect_bg.center))
        ...
        exp_text = self.ui_font.render(f"EXP: {self.player.exp}/{self.player.exp_to_next_level}", True, (255, 255, 255))
        self.screen.blit(exp_text, exp_text.get_rect(center=exp_bar_rect_bg.center))

        # --- NOVA BARRA DE COOLDOWN DA HABILIDADE ---
        ...
        pygame.draw.rect(self.screen, (80, 80, 80), skill_bar_bg)
        pygame.draw.rect(self.screen, (255, 215, 0), skill_bar_fg) # Cor dourada
        ...
        # Wave, Score e High Score
        wave_text = self.ui_font.render(f"Wave: {self.current_wave}", True, (255, 255, 255))
        score_text = self.ui_font.render(f"Score: {self.score}", True, (255, 255, 255))
        highscore_text = self.ui_font.render(f"Recorde: {self.high_score}", True, (255, 255, 0))
        self.screen.blit(wave_text, wave_text.get_rect(topright=(SCREEN_WIDTH - 10, 10)))
        self.screen.blit(score_text, score_text.get_rect(topright=(SCREEN_WIDTH - 10, 30)))
        self.screen.blit(highscore_text, highscore_text.get_rect(topright=(SCREEN_WIDTH - 10, 50)))
    ...
    def player_attack_logic(self):
        ...
        if isinstance(attack_result, tuple) and attack_result[0] == "BULLET":
            ...
            if sword_hitbox:
                for enemy in self.enemies[:]:
                    if sword_hitbox.colliderect(enemy.hitbox):
                        damage_dealt = self.player.weapon.damage
